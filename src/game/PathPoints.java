...
public class PathPoints {
    ...
    public PathPoints(Scanner s) {
        path = new ArrayList<Point>(); // creates new ArrayList of Coordinates
    ...
    public void drawLine(Graphics g) {
        ...
        for (int n = 0; n < path.size() - 1; n++) { // draws a segment then repeats
    ...
    public PathPosition getStart() {
        return new PathPosition(new ArrayList<Point>(path));
    }
    ...
    public double distanceToPath(double px, double py) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            ...
            // just compute the distance to that endpoint, otherwise use Pythagorean
            // theorem.
